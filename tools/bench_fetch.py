...
def main():
    ...
    for i in range(0, len(pmids), 80):
        q = " OR ".join(f"EXT_ID:{p}" for p in pmids[i:i + 80]) + " AND SRC:MED AND OPEN_ACCESS:Y"
        ...
        for r in res.get("resultList", {}).get("result", []):
            if r.get("pmcid") and r.get("id") in by_pmid:
                d = docs[by_pmid[r["id"]]]
                d.update(pmcid=r["pmcid"], license=r.get("license"), title=r.get("title"))
                open_docs[by_pmid[r["id"]]] = d
        print(f"  Europe PMC lookups {min(i + 80, len(pmids))}/{len(pmids)}: {len(open_docs)} open access", end="\r", flush=True)
    print()
